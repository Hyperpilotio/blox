...
def main(argv):
    ...
    args = [{'arg': '--apigateway', 'dest': 'apigateway', 'default': None,
             'type': 'boolean', 'help': 'Call API Gateway endpoint'}]
    if '--apigateway' in argv:
        args.extend([{'arg': '--stack', 'dest': 'stack', 'default': None,
                      'help': 'CloudFormation stack name'}])
    else:
        args.extend([{'arg': '--host',
                      'dest': 'host',
                      'default': 'localhost:2000',
                      'help': 'Blox Scheduler <Host>:<Port>'}])
    args.extend([{'arg': '--environment', 'dest': 'environment',
                  'default': None, 'help': 'Blox environment name'}])

    # Parse Command Line Arguments
    params = common.parse_cli_args('Delete Blox Environment', args)
...
def run_apigateway(params):
    ...
    command = ["cloudformation", "describe-stack-resource", "--stack-name",
    ...
    command = ["apigateway",
               "test-invoke-method",
               "--rest-api-id",
               restApi['StackResourceDetail']['PhysicalResourceId'],
               "--resource-id",
               ...
               "/v1/environments/{}".format(params.environment)]
...
def run_local(params):
    ...
    api.uri = '/v1/environments/{}'.format(params.environment)
